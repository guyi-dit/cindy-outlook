(function () {
  ...
  var REAUTH_MESSAGES = {
    'zh-CN': 'Microsoft 授权已失效，请重新连接账号。',
    en: 'Your Microsoft authorization has expired. Please reconnect your account.',
    ja: 'Microsoft の認証が期限切れです。アカウントを再接続してください。',
    ko: 'Microsoft 인증이 만료되었습니다. 계정을 다시 연결하세요.',
  };
  ...
  function connectError(result) {
    var labels = {
      NO_CLIENT_CONFIG: '还没写入 Azure 应用 ID。请先运行 scripts/set-client-id.py，再刷新插件',
      INVALID_CONFIG: 'OAuth 配置无效，请检查 ghost.json 里的 clientId 和重定向 URI',
      CALLBACK_INVALID: '授权回调校验失败，请重试',
      EXCHANGE_FAILED: 'Microsoft token 交换失败，请核对 clientId、重定向 URI 是否为 http://127.0.0.1:53683/callback',
      NETWORK: '连接 Microsoft 失败，请检查网络后重试',
      TIMEOUT: '授权等待超时，请重试',
      CANCELLED: '授权已取消',
      ACCOUNT_LIMIT: '已达到账号数量上限',
      VAULT_WRITE_FAILED: '账号保存失败，请重试',
      LISTEN_FAILED: '本机回跳端口 53683 被占用，请关闭占用该端口的程序后重试',
    };
  ...
  async function load() {
    try {
      ...
      render(entry);
      if (entry && entry.clientConfigured === false) {
        status('还没配置 Azure 应用 ID，请先按上方步骤写入 clientId');
      }
    } catch (_err) {
      render(null);
      status('账号状态加载失败，请重试');
